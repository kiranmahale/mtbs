class ReportsController < ApplicationController
  def reports
    ...
    @screening_details = ScreeningDetail.joins(:screen, :movie, :time_slot)
                                        .where(conditions)
  end
  ...
  def build_filter_conditions
    ...
    cond << ['time_slots.start_time >= ?', @start_time.to_i] if @start_time.present? && @start_time != '0'

    @end_time = params['filter_end_time'] || nil
    cond << ['time_slots.end_time <= ?', @end_time.to_i] if @end_time.present? && @end_time != '0'
